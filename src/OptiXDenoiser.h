...
public:
    ...
    // Resolution limits
    //
    // OptiX describes the denoiser's internal activation tensors with 32-bit
    // byte counts, so a single denoiser invocation can never address more than
    // 2^32-1 bytes of tensor. The HDR / temporal models allocate ~128 bytes of
    // tensor per pixel, which puts the hard ceiling for one invocation at
    // roughly 33.5 MP (about 8192x4096). Larger frames -- 8K full-height, 10K,
    // 16K -- must be split into tiles; the tiles are what get denoised, so the
    // frame itself can be arbitrarily large as long as it fits in VRAM.
    // ---------------------------------------------------------------------
    static constexpr uint64_t kTensorByteLimit   = 4294967295ull;
    static constexpr uint64_t kTensorBytesPerPx  = 128ull;
    static constexpr uint64_t kMaxDenoiserPixels = kTensorByteLimit / kTensorBytesPerPx;

    // Largest / smallest tile edge we will hand to OptiX. 4096 + overlap stays
    // comfortably inside the tensor limit; below 256 the overlap band dominates.
    static constexpr unsigned int kMaxTileEdge = 4096u;
    static constexpr unsigned int kMinTileEdge = 256u;

    // True when the frame cannot be denoised in a single invocation.
    static bool exceedsTensorLimit(uint64_t width, uint64_t height)
    {
        return (width * height) > kMaxDenoiserPixels;
    }

    // Clamps a requested tile size so that tile + overlap always stays inside
    // the tensor limit, and forces tiling on when the frame itself is too big.
    // requestedW/H of 0 means "no tiling requested".
    static void resolveTileSize(unsigned int imageWidth, unsigned int imageHeight,
                                unsigned int requestedWidth, unsigned int requestedHeight,
                                unsigned int& outWidth, unsigned int& outHeight);

    // Rough device-memory estimate for the persistent full-frame images.
    static uint64_t estimateDeviceBytes(unsigned int width, unsigned int height,
...
private:
    ...
    size_t                m_scratch_size = 0;   // size_t: can legitimately exceed 4 GB
...
public:
    // Tile size presets.
    // NOTE: new entries are appended, never inserted, so that the enumeration
    // labels stored in existing .nk scripts still resolve to the same value.
    enum TilePreset {
        TILE_NONE = 0,      // No tiling (full image) - only valid below ~33.5 MP
        TILE_512 = 1,       // 512x512 tiles
        TILE_1024 = 2,      // 1024x1024 tiles  
        TILE_2048 = 3,      // 2048x2048 tiles
        TILE_4096 = 4,      // 4096x4096 tiles - fewer seams on 8K/16K plates
        TILE_AUTO = 5       // Pick from the frame size and free VRAM
    };
...
private:
    ...
    // Helper methods
    bool validateCUDA();
    bool inputConnected(int n) const;
    void allocateBuffers();
    // Refuses the render with a readable message instead of dying inside CUDA
    // when the plate cannot fit in host or device memory.
    void preflightMemory(unsigned int width, unsigned int height) const;
    ...
    // Tile preset helpers
    void getTileDimensions(unsigned int imageWidth, unsigned int imageHeight,
