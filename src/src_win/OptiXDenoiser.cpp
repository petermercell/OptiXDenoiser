...
void OptiXDenoiserCore::resolveTileSize(unsigned int imageWidth, unsigned int imageHeight,
                                        unsigned int requestedWidth, unsigned int requestedHeight,
                                        unsigned int& outWidth, unsigned int& outHeight)
{
    const bool frameTooBig = exceedsTensorLimit(imageWidth, imageHeight);

    // "No tiling" is only honoured when the frame actually fits in one
    // invocation. Above ~33.5 MP we tile regardless of what the user asked for,
    // otherwise OptiX rejects the frame outright.
    if (requestedWidth == 0 || requestedHeight == 0) {
        ...
        requestedWidth  = static_cast<unsigned int>(kMaxTileEdge);
        requestedHeight = static_cast<unsigned int>(kMaxTileEdge);

        std::cerr << "[OptiXDenoiser] " << imageWidth << "x" << imageHeight
                  << " exceeds the single-pass limit of " << static_cast<unsigned long long>(kMaxDenoiserPixels)
                  << " pixels - falling back to " << static_cast<unsigned int>(kMaxTileEdge)
                  << "x" << static_cast<unsigned int>(kMaxTileEdge)
                  << " tiles." << std::endl;
    }

    outWidth  = std::min(requestedWidth,  imageWidth);
    outHeight = std::min(requestedHeight, imageHeight);

    const unsigned int minEdge = static_cast<unsigned int>(kMinTileEdge);
    const unsigned int maxEdge = static_cast<unsigned int>(kMaxTileEdge);

    outWidth  = std::min(std::max(outWidth,  minEdge), maxEdge);
    outHeight = std::min(std::max(outHeight, minEdge), maxEdge);

    // A tile never needs to be bigger than the frame.
    outWidth  = std::min(outWidth,  imageWidth);
    outHeight = std::min(outHeight, imageHeight);

    // Shrink until the tile plus a generous overlap allowance is inside the
    // tensor limit. In practice this never fires for edges <= 4096, but it keeps
    // the invariant true if kMaxTileEdge is ever raised.
    while (outWidth > kMinTileEdge && outHeight > kMinTileEdge &&
           exceedsTensorLimit(static_cast<uint64_t>(outWidth) + 256ull,
                              static_cast<uint64_t>(outHeight) + 256ull)) {
        outWidth  = std::max(minEdge, outWidth  / 2u);
        outHeight = std::max(minEdge, outHeight / 2u);
    }
}

uint64_t OptiXDenoiserCore::estimateDeviceBytes(unsigned int width, unsigned int height,
                                                bool hasAlbedo, bool hasNormal,
                                                bool temporal, bool hasFlow)
{
    const uint64_t pixels = static_cast<uint64_t>(width) * static_cast<uint64_t>(height);
    const uint64_t rgba   = pixels * 4ull * sizeof(float);

    uint64_t total = rgba * 2ull;                 // input + output
    if (temporal)             total += rgba;      // previousOutput
    if (hasAlbedo)            total += rgba;
    if (hasNormal)            total += rgba;
    if (temporal && hasFlow)  total += pixels * 2ull * sizeof(float);
...
void OptiXDenoiserCore::validateHDRRange(const float* imageData, size_t pixelCount, bool& outOfRange)
{
    ...
    // This is a diagnostic, not a correctness check, so sample rather than walk
    // every pixel -- a full scan of a 16K plate costs more than the denoise.
    const size_t maxSamples = 4u << 20;   // 4M samples is plenty to spot a problem
    const size_t stride = (pixelCount > maxSamples) ? (pixelCount / maxSamples) : 1;

    for (size_t i = 0; i < pixelCount; i += stride) {
        const float r = imageData[i * 4 + 0];
        const float g = imageData[i * 4 + 1];
        const float b = imageData[i * 4 + 2];
    ...
    if (outOfRangeCount > 0) {
        outOfRange = true;
        std::cerr << "Warning: pixels exceeding the recommended HDR range [0-10000] were found"
                  << (stride > 1 ? " (sampled)" : "")
                                                   ...
                                                   const float* hmem, OptixPixelFormat format)
{
    ...
    // 64-bit throughout: a 16K RGBA float plane is already 2.1 GB, and the
    // pixel count alone overflows 32 bits past ~16384x16384.
    const size_t rowBytes = static_cast<size_t>(width) * pixelSize;
    const size_t frame_byte_size = rowBytes * static_cast<size_t>(height);
    size_t actualSize = 0;

    // OptixImage2D::rowStrideInBytes is a 32-bit field.
    if (rowBytes > static_cast<size_t>(UINT32_MAX)) {
        throw std::runtime_error("Image row stride exceeds 32 bits (width too large)");
    }
    ...
    oi.width = width;
    oi.height = height;
    oi.rowStrideInBytes = static_cast<unsigned int>(rowBytes);
    oi.pixelStrideInBytes = static_cast<unsigned int>(pixelSize);
    oi.format = format;
...
void OptiXDenoiserCore::copyOptixImage2D(OptixImage2D& dest, const OptixImage2D& src)
{
    ...
    const size_t copySize = static_cast<size_t>(src.width) * static_cast<size_t>(src.height) * pixelSize;
...
void OptiXDenoiserCore::init(const Data& data, unsigned int tileWidth, unsigned int tileHeight, bool temporalMode)
{
    ...
    if (cuResult != CUDA_SUCCESS || cuContext == nullptr) {
        ...
        // CUDA 13.x remaps cuCtxCreate -> cuCtxCreate_v4, which added an
        // execution-affinity params array/count that we don't need. Call the
        // stable v2 ABI entry point explicitly so the old 3-arg call keeps
        // working whether we're built against CUDA 12.x or 13.x headers.
        // CUDA 13.0 removed the old cuCtxCreate_v2/_v3 symbols from cuda.h and
        // made cuCtxCreate_v4 the only implementation, with a new signature/
        // argument order: (pctx, ctxCreateParams, flags, dev). Branch so this
        // builds against both pre-13.0 and 13.0+ toolkits.
#if CUDA_VERSION >= 13000
        cuResult = cuCtxCreate(&cuContext, nullptr, 0, cuDevice);
#else
    ...
    // Tiling setup.
    //
    // optixDenoiserComputeMemoryResources validates the internal tensor for the
    // dimensions it is given, so it MUST be called with the tile size. Calling
    // it with the full frame makes OptiX reject anything above ~33.5 MP
    // ("input resolution too large, tensor size N") before the tiled invoke path
    // ever runs -- which is exactly what breaks 8K+, 10K and 16K plates.
    // ---------------------------------------------------------------------
    resolveTileSize(data.width, data.height, tileWidth, tileHeight,
                    m_tileWidth, m_tileHeight);

    const bool tiled = (m_tileWidth < data.width) || (m_tileHeight < data.height);

    OptixDenoiserSizes denoiser_sizes = {};
    OPTIX_CHECK(optixDenoiserComputeMemoryResources(m_denoiser, m_tileWidth, m_tileHeight,
                                                    &denoiser_sizes));

    if (!tiled) {
        m_overlap      = 0;
        m_scratch_size = denoiser_sizes.withoutOverlapScratchSizeInBytes;
    } else {
        m_overlap      = denoiser_sizes.overlapWindowSizeInPixels;
        m_scratch_size = denoiser_sizes.withOverlapScratchSizeInBytes;

        // The overlap band has to fit inside the tile. Grow the tile rather than
        // falling back to full-frame denoising -- on a 16K plate that fallback
        // would put us straight back over the tensor limit.
        const unsigned int minTile = std::max(static_cast<unsigned int>(kMinTileEdge), 4u * m_overlap);
        if (m_tileWidth < minTile || m_tileHeight < minTile) {
            m_tileWidth  = std::min(std::max(m_tileWidth,  minTile), data.width);
            m_tileHeight = std::min(std::max(m_tileHeight, minTile), data.height);
    ...
    // Final guard: tile + overlap band must still be inside the tensor limit.
    {
        const uint64_t setupW = static_cast<uint64_t>(m_tileWidth)  + 2ull * m_overlap;
        const uint64_t setupH = static_cast<uint64_t>(m_tileHeight) + 2ull * m_overlap;
        if (exceedsTensorLimit(setupW, setupH)) {
            throw std::runtime_error(
                "Tile size " + std::to_string(m_tileWidth) + "x" + std::to_string(m_tileHeight) +
                " (+" + std::to_string(2 * m_overlap) + " px overlap) still exceeds the OptiX "
                "denoiser tensor limit of " + std::to_string(static_cast<unsigned long long>(kMaxDenoiserPixels)) +
                " pixels. Choose a smaller tile preset.");
        }
    }

    m_state_size = denoiser_sizes.stateSizeInBytes;

    // optixDenoiserComputeIntensity / ComputeAverageColor run over the WHOLE
    // frame, not a tile, but share this scratch buffer. Their requirement is a
    // small fixed-size reduction workspace; the floor below keeps them safe now
    // that the scratch size is derived from the tile.
    m_scratch_size = std::max(m_scratch_size, static_cast<size_t>(16u << 20));

    // Pre-flight the device allocation so a 16K plate on an undersized card
    // reports a readable message instead of an opaque CUDA out-of-memory.
    {
        size_t freeMem = 0, totalMem = 0;
        if (cudaMemGetInfo(&freeMem, &totalMem) == cudaSuccess) {
            const uint64_t needed =
                estimateDeviceBytes(data.width, data.height,
                                    data.albedo != nullptr, data.normal != nullptr,
                                    m_temporalMode, data.flow != nullptr) +
                m_scratch_size + m_state_size;

            if (needed > static_cast<uint64_t>(freeMem)) {
                const double neededGB = static_cast<double>(needed) / (1024.0 * 1024.0 * 1024.0);
                const double freeGB   = static_cast<double>(freeMem) / (1024.0 * 1024.0 * 1024.0);
                char msg[512];
                snprintf(msg, sizeof(msg),
                         "Not enough GPU memory for %ux%u: needs ~%.2f GB, %.2f GB free. "
                         "Disconnect the albedo/normal/motion inputs or denoise a smaller region.",
                         data.width, data.height, neededGB, freeGB);
    ...
    m_state = reinterpret_cast<CUdeviceptr>(allocateAlignedCudaMemory(m_state_size, actualSize, "state"));
...
void OptiXDenoiserCore::update(const Data& data)
{
    ...
    const size_t colorBufferSize = pixelCount * sizeof(float4);
    ...
    if (data.flow && m_guideLayer.flow.data && m_temporalMode) {
        const size_t flowBufferSize = pixelCount * 2 * sizeof(float);
...
void OptiXDenoiserCore::exec()
{
    ...
    // Keyed off m_overlap so this can never disagree with how the scratch buffer
    // was sized in init() -- the tiled path needs withOverlapScratchSizeInBytes.
    const bool useTiling = (m_overlap > 0) &&
                           (m_tileWidth  < m_layers[0].input.width ||
                            m_tileHeight < m_layers[0].input.height);
...
void OptiXDenoiserCore::getResults()
{
    ...
    const size_t frame_byte_size = static_cast<size_t>(m_layers[0].output.width) *
                                   static_cast<size_t>(m_layers[0].output.height) * sizeof(float4);
...
void OptiXDenoiserIop::logTileChange(int oldPreset, int newPreset) const
{
    ...
    getTileDimensions(m_width, m_height, oldW, oldH);
    
    // Get new dimensions
    const_cast<OptiXDenoiserIop*>(this)->m_tilePreset = newPreset;
    getTileDimensions(m_width, m_height, newW, newH);
...
OptiXDenoiserIop::OptiXDenoiserIop(Node *node) : PlanarIop(node)
{
    ...
    // Tile preset - Auto adapts to frame size and free VRAM, and is the only
    // setting that is correct for both HD and 16K without user intervention.
    m_tilePreset = TILE_AUTO;
...
void OptiXDenoiserIop::getTileDimensions(unsigned int imageWidth, unsigned int imageHeight,
                                         unsigned int& tileWidth, unsigned int& tileHeight) const
{
    switch (m_tilePreset) {
        case TILE_512:
            tileWidth = 512;
        ...
        case TILE_4096:
            ...
            break;
        case TILE_AUTO: {
            // Small enough for one pass -> no tiling at all.
            if (imageWidth && imageHeight &&
                !OptiXDenoiserCore::exceedsTensorLimit(imageWidth, imageHeight)) {
                tileWidth = 0;
                tileHeight = 0;
                break;
            }

            // Otherwise pick the largest tile the card can comfortably scratch.
            // Bigger tiles mean fewer overlap re-computations and fewer seams.
            unsigned int edge = 2048;
            size_t freeMem = 0, totalMem = 0;
            if (cudaMemGetInfo(&freeMem, &totalMem) == cudaSuccess) {
                const double freeGB = static_cast<double>(freeMem) / (1024.0 * 1024.0 * 1024.0);
                if      (freeGB >= 16.0) edge = 4096;
                else if (freeGB >=  8.0) edge = 2048;
                else if (freeGB >=  4.0) edge = 1024;
                else                     edge = 512;
            }
            tileWidth = edge;
            tileHeight = edge;
        ...
        case TILE_NONE:
        default:
            // 0 means "ask for no tiling"; the core still forces tiling on if
            // the frame is above the single-pass limit.
            tileWidth = 0;
            tileHeight = 0;
            break;
    }
...
const char* OptiXDenoiserIop::getTilePresetName(int preset) const
{
    switch (preset) {
        ...
        case TILE_4096: return "4096x4096 Tiles";
        case TILE_AUTO: return "Auto";
...
void OptiXDenoiserIop::knobs(Knob_Callback f)
{
    ...
    // Add frame status info
    Text_knob(f, "Frame Info");
    SetFlags(f, Knob::STARTLINE);
    
    Divider(f, "Memory Settings");
    
    static const char* tile_preset_names[] = {"No Tiling", "512x512", "1024x1024",
                                              "2048x2048", "4096x4096", "Auto", nullptr};
    ...
    Tooltip(f, "Tile size preset for memory optimization\n"
             "Auto: pick from frame size and free VRAM (recommended)\n"
             "No Tiling: single pass - only valid up to ~33.5 MP (approx 8192x4096)\n"
             "512x512: good for 8GB VRAM\n"
             "1024x1024: good for 12GB+ VRAM\n"
             "2048x2048: good for 24GB+ VRAM\n"
             "4096x4096: fewest seams on 8K/16K plates, 16GB+ free VRAM\n"
             "\n"
             "Frames above ~33.5 MP (8K full-height, 10K, 16K) exceed the OptiX\n"
             "denoiser's single-pass tensor limit and are always tiled, whatever\n"
             "this is set to.");
    SetFlags(f, Knob::STARTLINE);

    Divider(f, "");
    
    Text_knob(f, "OptiXDenoiser by Peter Mercell v1.03 / 2025");
    SetFlags(f, Knob::STARTLINE);
}
...
void OptiXDenoiserIop::allocateBuffers()
{
    ...
    // Only allocate what is actually wired up. Each RGBA float plane is 2.1 GB
    // at 16K, so allocating unused albedo/normal/motion buffers would waste
    // ~7 GB of host RAM per node.
    try {
        m_colorBuffer.resize(bufferSizeRGBA, 0.0f);
        m_outputBuffer.resize(bufferSizeRGBA, 0.0f);

        if (inputConnected(1)) m_albedoBuffer.resize(bufferSizeRGBA, 0.0f);
        if (inputConnected(2)) m_normalBuffer.resize(bufferSizeRGBA, 0.0f);

        if (inputConnected(3) && m_bTemporal) {
            m_motionBuffer.resize(bufferSizeRGBA, 0.0f);
            m_motionXYBuffer.resize(bufferSizeXY, 0.0f);
        }
    } catch (const std::bad_alloc& e) {
        const double gb = static_cast<double>(bufferSizeRGBA * sizeof(float)) / (1024.0 * 1024.0 * 1024.0);
        char msg[256];
        snprintf(msg, sizeof(msg),
                 "Failed to allocate host buffers for %ux%u (%.2f GB per plane): %s",
                 m_width, m_height, gb, e.what());
        throw std::runtime_error(msg);
    }
}

bool OptiXDenoiserIop::inputConnected(int n) const
{
    return (node_inputs() > n) && (input(n) != nullptr);
}

void OptiXDenoiserIop::preflightMemory(unsigned int width, unsigned int height) const
{
    const uint64_t pixels = static_cast<uint64_t>(width) * static_cast<uint64_t>(height);
    if (pixels == 0) {
        throw std::runtime_error("Invalid processing area");
    }

    // std::vector<float> indexes with size_t; on a 64-bit build this is only a
    // real constraint for absurd resolutions, but the check is free.
    if (pixels > (SIZE_MAX / (4 * sizeof(float)))) {
        throw std::runtime_error("Processing area too large to address");
    }

    if (OptiXDenoiserCore::exceedsTensorLimit(width, height) && m_tilePreset == TILE_NONE) {
        std::cerr << "[OptiXDenoiser] Tile Size is set to \"No Tiling\" but "
                  << width << "x" << height << " is above the single-pass limit - "
                  << "tiling automatically." << std::endl;
    }
}
...
void OptiXDenoiserIop::readInputPlanes()
{
    ...
    for (int i = 0; i < node_inputs(); ++i) {
        ...
        try {
            ...
            // Select target buffer. Buffers for unconnected or unused inputs are
            // deliberately left unallocated, so check emptiness rather than the
            // raw data() pointer.
            std::vector<float>* target = nullptr;
            switch (i) {
            case 0: target = &m_colorBuffer;  break;
            case 1: target = &m_albedoBuffer; break;
            case 2: target = &m_normalBuffer; break;
            case 3: target = &m_motionBuffer; break;
            }

            float* targetBuffer = (target && !target->empty()) ? target->data() : nullptr;

            if (targetBuffer) {
                const size_t chanStride = static_cast<size_t>(inputPlane.chanStride());
                ...
                // Hoist the per-channel base pointers out of the pixel loop and
                // intersect the two rectangles once. The original form
                // recomputed a pointer and re-tested bounds for every channel of
                // every pixel, which is ~530M redundant operations on a 16K
                // plate.
                const float* planeData = inputPlane.readable();
                const float* chanPtr[4] = { nullptr, nullptr, nullptr, nullptr };
                for (int c = 0; c < 4; ++c) {
                    if (channelMap[c] >= 0) {
                        chanPtr[c] = planeData + chanStride * static_cast<size_t>(channelMap[c]);
                    }
                }

                const int x0 = std::max(planeX, bufferX);
                const int y0 = std::max(planeY, bufferY);
                const int x1 = std::min(planeX + planeWidth,  bufferX + static_cast<int>(m_width));
                const int y1 = std::min(planeY + planeHeight, bufferY + static_cast<int>(m_height));

                for (int y = y0; y < y1; ++y) {
                    if (aborted() || cancelled()) return;

                    const size_t srcRow = static_cast<size_t>(y - planeY) * static_cast<size_t>(planeWidth);
                    const size_t dstRow = static_cast<size_t>(y - bufferY) * static_cast<size_t>(m_width);

                    for (int x = x0; x < x1; ++x) {
                        const size_t si = srcRow + static_cast<size_t>(x - planeX);
                        float* dst = targetBuffer + (dstRow + static_cast<size_t>(x - bufferX)) * 4;

                        for (int c = 0; c < 4; ++c) {
                            if (chanPtr[c]) {
                                const float value = chanPtr[c][si];
                                dst[c] = std::isfinite(value) ? value : 0.0f;
                            } else {
    ...
    if (inputConnected(2) && !m_normalBuffer.empty()) {
        m_denoiser->clampNormalBuffer(m_normalBuffer.data(), pixelCount);
    }

    // Validate HDR range
    if (!m_colorBuffer.empty()) {
        bool outOfRange = false;
        m_denoiser->validateHDRRange(m_colorBuffer.data(), pixelCount, outOfRange);
    }

    // Convert motion vectors if needed
    if (inputConnected(3) && m_bTemporal &&
        !m_motionBuffer.empty() && !m_motionXYBuffer.empty()) {
        m_denoiser->convertMotionVectors(m_motionBuffer.data(), m_motionXYBuffer.data(),
                                         m_width, m_height);
    }
}
...
void OptiXDenoiserIop::writeOutputPlane(ImagePlane& plane)
{
    ...
    // Direct copy - no coordinate mapping needed. The dimension check above
    // already guarantees every index is in range, so the per-pixel bounds test
    // that used to live in the inner loop is hoisted into an assert-style guard.
    const size_t expected = static_cast<size_t>(m_width) * static_cast<size_t>(m_height) * 4;
    if (m_outputBuffer.size() < expected) {
        throw std::runtime_error("Output buffer smaller than the processed area");
    }

    float* planeBase = plane.writable();
    const size_t chanStride = static_cast<size_t>(plane.chanStride());

    for (int chanNo = 0; chanNo < 4; chanNo++) {
        float* outdata = planeBase + chanStride * static_cast<size_t>(chanNo);

        for (int y = 0; y < planeHeight; y++) {
            const float* src = outputData + static_cast<size_t>(y) * static_cast<size_t>(m_width) * 4 + chanNo;
            float* dst = outdata + static_cast<size_t>(y) * static_cast<size_t>(planeWidth);

            for (int x = 0; x < planeWidth; x++) {
                dst[x] = src[static_cast<size_t>(x) * 4];
            }
        }
    }
}
...
void OptiXDenoiserIop::renderStripe(ImagePlane& plane)
{
    ...
    getTileDimensions(processWidth, processHeight, tileWidth, tileHeight);
    ...
    try {
        // Fail loudly and readably before touching CUDA if the plate can't fit.
        preflightMemory(m_width, m_height);
        ...
        data.albedo = m_albedoBuffer.empty()   ? nullptr : m_albedoBuffer.data();
        data.normal = m_normalBuffer.empty()   ? nullptr : m_normalBuffer.data();
        data.flow   = (m_bTemporal && !m_motionXYBuffer.empty()) ? m_motionXYBuffer.data() : nullptr;
